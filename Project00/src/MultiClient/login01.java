			...
			public void actionPerformed(ActionEvent e) {
				...
				if (IdField.getText().isEmpty() == true) {
					...
					try {
						...
						//------------------------ 찾는 값이 arraylist 에 없을 때(-1)
						if(a0.indexOf(IdField.getText())==-1){
								JOptionPane.showMessageDialog(null, "id 또는 pw 가 다릅니다.", "입력",
										JOptionPane.ERROR_MESSAGE);
						}else{
							if (!((a0.get(a0.indexOf(IdField.getText())).equals(IdField.getText())) && (a1.get(a1.indexOf(PwField.getText())).equals(PwField.getText())))) {
								JOptionPane.showMessageDialog(null, "id 또는 pw 가 다릅니다.", "입력",
									JOptionPane.ERROR_MESSAGE);
								IdField.requestFocus();
							} else {
								dispose(); // 현재 열린 창 닫기
								getContentPane().add(new testClient02()); // client05 생성자 호출(client 폼 불러오기)
							}
						}
						a0.clear();
						a1.clear();
